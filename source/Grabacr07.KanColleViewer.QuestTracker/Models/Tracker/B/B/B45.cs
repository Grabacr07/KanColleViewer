	...
	internal class B45 : ITracker
	{
		...
		public void RegisterEvent(TrackManager manager)
		{
			manager.BattleResultEvent += (sender, args) =>
			{
				...
				if ((ships[0]?.Info.Id ?? 0) != 437) return; // 기함 아카츠키改2
		...
		{
			return count >= max_count ? "완료" : "아카츠키改2 기함,히비키,이카즈치,이나즈마 만으로 편성 1-5 보스전 A승리 이상 " + count.ToString() + " / " + max_count.ToString();
		}
