	...
	internal class B76 : ITracker
	{
		...
		public void RegisterEvent(TrackManager manager)
		{
			manager.BattleResultEvent += (sender, args) =>
			{
				...
				if (flagship != 7) return; // 기함 경모
				if (ships.Count(x => x.Info.ShipType.Id == 2) < 3) return; // 구축 3척
		...
		{
			return count >= max_count ? "완료" : "경공모 기함,구축 3척 포함 편성 1-3 보스전 S승리 " + count.ToString() + " / " + max_count.ToString();
		}
