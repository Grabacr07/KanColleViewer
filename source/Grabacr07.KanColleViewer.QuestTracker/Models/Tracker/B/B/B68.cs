	...
	internal class B68 : ITracker
	{
		...
		public void RegisterEvent(TrackManager manager)
		{
			manager.BattleResultEvent += (sender, args) =>
			{
				...
				if (args.MapWorldId != 4 || args.MapAreaId != 1) return; // 4-1
				if (args.EnemyName != "東方派遣艦隊") return; // boss
				...
				if (ships.Count(x => x.Info.ShipType.Id == 2) < 4) return; // 카와카제,시구레 제외 구축 2척
		...
		{
			return count >= max_count ? "완료" : "센다이改2,카와카제改2,시구레改2,구축 2척 포함 편성 4-1 보스전 A승리 이상 " + count.ToString() + " / " + max_count.ToString();
		}
